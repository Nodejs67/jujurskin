...
const SAFETY_STYLES: Record<string, { color: string; bg: string; border: string; icon: React.ElementType }> = {
  sangat_aman: { color: "text-green-400", bg: "bg-green-400/10", border: "border-green-400/20", icon: CheckCircle },
  aman: { color: "text-blue-400", bg: "bg-blue-400/10", border: "border-blue-400/20", icon: CheckCircle },
  hati_hati: { color: "text-yellow-400", bg: "bg-yellow-400/10", border: "border-yellow-400/20", icon: AlertTriangle },
};
      ...
      <div className="max-w-2xl mx-auto px-6 py-10 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="pt-2">
          <div className="flex items-center gap-2 mb-2">
            ...
            {ing.pregnancy_safe && (
              <Badge variant="outline" className="text-xs border-green-400/30 text-green-400 bg-green-400/10 gap-1">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{ delay: 0.1 }}
          ...
          <div className={`rounded-xl border p-3 text-center ${ing.beginner_friendly ? "bg-amber-400/5 border-amber-400/20" : "bg-muted/30 border-border"}`}>
            <Star className={`w-4 h-4 mx-auto mb-1 ${ing.beginner_friendly ? "text-amber-400" : "text-muted-foreground"}`} />
            <p className={`text-[10px] font-semibold leading-tight ${ing.beginner_friendly ? "text-amber-400" : "text-muted-foreground"}`}>
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{ delay: 0.25 }}
          ...
          <div className="rounded-xl border border-green-400/20 bg-green-400/5 p-5">
            <p className="text-xs text-green-400 font-semibold uppercase tracking-wide mb-3">✅ Cocok untuk</p>
          ...
          {ing.avoid_if.length > 0 ? (
            ...
            <div className="rounded-xl border border-green-400/20 bg-green-400/5 p-5 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-xs text-green-400 font-medium">Tidak ada kontraindikasi</p>
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{ delay: 0.35 }}
          ...
          <div className={`rounded-xl border p-5 ${ing.conflicts_with.length > 0 ? "border-yellow-400/20 bg-yellow-400/5" : "border-border bg-card"}`}>
            <p className={`text-xs font-semibold uppercase tracking-wide mb-3 ${ing.conflicts_with.length > 0 ? "text-yellow-400" : "text-muted-foreground"}`}>
              ...
              {ing.myths.map((myth, i) => (
                <div key={i} className="rounded-xl border border-border bg-card overflow-hidden">
                  ...
                  <div className="flex items-start gap-3 p-4 bg-green-400/5">
                    ...
                    <div>
                      <p className="text-xs text-green-400 font-medium mb-0.5 uppercase tracking-wide">Fakta</p>
